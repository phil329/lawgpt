...
from utils.tools import transverse_on_json,extract_json_from_string,json2file,get_project_path
...
if 'yuangao_data' not in st.session_state:
    st.session_state['yuangao_data'] ={'姓名': None, '性别': None, '出生日期': None, '民族': None, '住址': None, '联系方式': None, '身份证号':None, '委托诉讼代理人': {'姓名': None, '事务所': None}}

if 'beigao_data' not in st.session_state:
    st.session_state['beigao_data'] ={'姓名': None, '性别': None, '出生日期': None, '民族': None, '住址': None, '联系方式': None, '身份证号':None, '委托诉讼代理人': {'姓名': None, '事务所': None}}

if 'yuangao_company_data' not in st.session_state:
    st.session_state['yuangao_company_data'] ={ "公司名称": None, "公司所在地":  None,"法定代表人/法定代理人/法人": { "姓名": None, "职务": None, "联系方式": None},"委托诉讼代理人": {"姓名": None,"事务所": None}}

if 'beigao_company_data' not in st.session_state:
    st.session_state['beigao_company_data'] ={ "公司名称": None, "公司所在地":  None,"法定代表人/法定代理人/法人": { "姓名": None, "职务": None, "联系方式": None},"委托诉讼代理人": {"姓名": None,"事务所": None}}
...
if 'third_state_data' not in st.session_state:
    st.session_state['third_state_data'] = {"案由": None, "诉讼请求": None, "事实理由": None, "证据": None, "法院":None, "日期": None}

if 'third_state_step' not in st.session_state:
    st.session_state['third_state_step'] = 0
...
if 'agent_flag' not in st.session_state:
...
def excute_third():
    ...
    # 判断案由
    if st.session_state['third_state_step'] == 0:  
        if st.session_state['category']=="我不太清楚诶":
            res_answer = "这边检测到您还没有选择**案由**，请在左边选择您的案由，如果您不清楚可以向我描述一下，我会帮你**智能识别**案由"
            st.session_state['third_state_data']["案由"]="机动车交通事故责任纠纷"
            return res_answer
        else:
    ...
    # 输入诉讼请求
    elif st.session_state['third_state_step'] == 1:  
        res_answer="好的，我已经知道您的上诉请求了。**"+ st.session_state['category'] +"**的**诉讼请求**提示如下：\n\n"
        if st.session_state['category'] == "民间借贷":
    ...
    # 输入事实和理由
        ...
        res_answer = "如下是为您推荐的法院：" + recommend + "\n\n**请选择您将上诉的法院。**"
        st.session_state['third_state_step'] = 4
    
    # 填写法院名称
    elif st.session_state['third_state_step'] == 4:   
...
def excute_second():
    if st.session_state['is_person']==False and st.session_state['is_company']==False:
        ...
        if res_1_2==1:
            ...
            res_answer="好的，请先告诉我你的被告公司名称、公司所在地、公司法定代表人/法定代理人/法人、公司委托诉讼代理人的姓名和事务所"
            st.session_state['gen_keymiss'] =["公司名称", "公司所在地","法定代表人/法定代理人/法人","委托诉讼代理人"]
            st.session_state['gen_keyget'] =[]
        else:
            st.session_state['is_person']=False
            st.session_state['is_company']=False
...
def excute_first():
    if st.session_state['is_person']==False and st.session_state['is_company']==False:
        ...
        check_miss(data)
        if len(st.session_state['gen_keymiss'])!=0:
            res_answer="现在还不知道您公司的**"+'，'.join(st.session_state['gen_keymiss'])+"**信息，您能告诉我吗？"
        else:
...
st.text_area(label='用户输入框：',on_change=on_input_change,key='user_input')
...
for i in range(len(st.session_state['current_chat'])-1,-1,-1):
    ...
    if st.session_state["is_audio"] and msg["role"]!='user':
        # if msg["role"]!='user':
        wav_bytes = text2audio(msg['content'],os.path.join(project_path,'output','audio_folder','output.wav'))
