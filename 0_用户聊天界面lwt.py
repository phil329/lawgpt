...
def excute_third():
    ...
    if st.session_state['category']=="我不太清楚诶" and st.session_state['third_state_data']["案由"]==None:
        res_answer="这边检测到您还没有选择**案由**，您可以在左边选择案由，若不清楚案件类型请简单描述一下案件情况，我会帮你**智能识别**案由"
...
def excute_second():
    if st.session_state['is_person']==False and st.session_state['is_company']==False:
        ...
        if judge_p_c(res_judge_no_2) or st.session_state['third_state_data']["案由"]=="不清楚":
            res_json=2
...
start_button = Button(label='SPEAK', button_type='success', margin = (5, 5, 5, 5), width=500)
...
if audio_result:
    ...
    #         placeholder2.image(os.path.join(project_path,'assert','end.jpg'))
    #         # st.session_state['audio_input']['content'] = ''
    #     elif audio_result.get("GET_ONREC") == 'running':
    #         placeholder2.image(os.path.join(project_path,'assert','sine_wave.gif'))
...
st.text_area(label='',on_change=on_input_change,key='user_input')
st.button('发送信息',key='is_sent')
